...
class BasicTrainer:
    ...
    def load_opt(self):
        ...
        if 'scheduler' in opt_checkpoint:
            self.scheduler.load_state_dict(opt_checkpoint['scheduler'])
        self.epoch = opt_checkpoint['epoch'] + 1   # resume from next epoch
        self.iter = opt_checkpoint.get('iter', 0)  # restore global iter counter
        print(f'finish loading opt: epoch={self.epoch} iter={self.iter}')
